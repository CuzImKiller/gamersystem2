...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = [`${client.guilds.size} Server`,'!help','Type !help',`${client.users.size} Members`,'!inv','By: Abdullah | CuzImKiller'];
    var i = -1;
    var j = 0;
...
//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
	...
	.setDescription(`:notes: الان يتم تشغيل : **${serverQueue.songs[0].title}**`)
		...
		//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
		if (!serverQueue) return msg.channel.send('لا يوجد شيء حالي ف العمل.');
		let index = 0;
		//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
...
//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
	...
	} else {//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
...
function play(guild, song) {
	...
	if (!song) {//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
		serverQueue.voiceChannel.leave();
		queue.delete(guild.id);
		return;//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
	}//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
	console.log(serverQueue.songs);
//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
	const dispatcher = serverQueue.connection.playStream(ytdl(song.url))
		.on('end', reason => {//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
			if (reason === 'Stream is not generating quickly enough.') console.log('Song ended.');
			else console.log(reason);
			serverQueue.songs.shift();//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
			play(guild, serverQueue.songs[0]);
		})//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
		.on('error', error => console.error(error));//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 5);//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'

	serverQueue.textChannel.send(`بدء تشغيل : **${song.title}**`);
}//by ,$ ReBeL ء , ??#4777 'CODES SERVER'

const adminprefix = "$vip";//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
client.on('message', message => {//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
  var argresult = message.content.split(` `).slice(1).join(' ');//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
    if (!devs.includes(message.author.id)) return;//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
if (message.content.startsWith(adminprefix + 'setgame')) {//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
...
client.on("message", message => {
 if (message.content === `${prefix}`) {
  const embed = new Discord.RichEmbed() //by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
      .setColor("#000000")//by , 𝓥𝓝 | ! CuzImKiller | كيلر#9999 ء ,  'CODES SERVER'
      .setDescription(`
${prefix}play ? لتشغيل أغنية برآبط أو بأسم
${prefix}skip ? لتجآوز الأغنية الحآلية
${prefix}pause ? إيقآف الأغنية مؤقتا
...
client.on('guildMemberAdd', member => {
    member.createDM().then(function (channel) {
return channel.send("**افضل سيرفر ممكن تتخيله  https://discord.gg/qv4kcyA**")
    }
    )});
